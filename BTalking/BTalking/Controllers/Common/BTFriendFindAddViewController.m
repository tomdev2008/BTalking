...
@interface BTFriendFindAddViewController ()<SSSearchBarDelegate>
...
@implementation BTFriendFindAddViewController
...
- (void)load_contacts
{
    ...
    NSString *strURL = [@"" stringByAppendingFormat:@"%@%@%@", @"http://",delegate._server, @"/contact" ];
...
-(void)set_people_values:(NSMutableArray *)users requestId:(NSInteger)requestId
{
    ...
    NSString *strURL = [@"" stringByAppendingFormat:@"%@%@%@", @"http://",delegate._server, @"/pchats"];
    NSURL *url = [NSURL URLWithString:strURL];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSDictionary *contact = self.dataSource[indexPath.row];
    
    BTFriendTableViewCell *cell = (BTFriendTableViewCell *)[self.tableView cellForRowAtIndexPath:indexPath];
    UIButton *sender = cell.btn_checked;

    [sender sendActionsForControlEvents: UIControlEventTouchUpInside];
    if(sender.selected)
    {
        [self add_contact:[contact objectForKey:@"_id"]];
    }
    else
    {
        [self del_contact:[contact objectForKey:@"_id"]];
    }
...
{
    [self.delegate on_result_value:nil requestId:self.mResponseId];
    [self.navigationController popViewControllerAnimated:YES];
}

-(void) add_contact:(NSString *) userid
{
    AppDelegate *delegate=(AppDelegate*)[[UIApplication sharedApplication]delegate];
    
    NSString *strURL = [@"" stringByAppendingFormat:@"%@%@%@%@", @"http://",delegate._server, @"/user/addfriend/", userid];
    NSURL *url = [NSURL URLWithString:strURL];
//    ASIHTTPRequest *request = [ASIHTTPRequest requestWithURL:url];
//    [request setUseCookiePersistence : YES];
//    [request addRequestHeader:@"X-Requested-With" value:@"XMLHttpRequest"];
...
//    
//    request.delegate = self;
//    request.didFinishSelector = @selector(add_contact_finished:);
//    request.didFailSelector = @selector(add_contact_failed:);
//    [request startAsynchronous];
...
-(void) del_contact:(NSString *) userid
{
    AppDelegate *delegate=(AppDelegate*)[[UIApplication sharedApplication]delegate];
    
    NSString *strURL = [@"" stringByAppendingFormat:@"%@%@%@%@", @"http://",delegate._server, @"/user/removefriend/", userid];
    NSURL *url = [NSURL URLWithString:strURL];
    ASIHTTPRequest *request = [ASIHTTPRequest requestWithURL:url];
    [request setUseCookiePersistence : YES];
    [request addRequestHeader:@"X-Requested-With" value:@"XMLHttpRequest"];
    ...
    request.delegate = self;
    request.didFinishSelector = @selector(del_contact_finished:);
    request.didFailSelector = @selector(del_contact_failed:);
    [request startAsynchronous];
}

- (void)add_contact_finished:(ASIHTTPRequest *)request
{
}

- (void)add_contact_failed:(ASIHTTPRequest *)request
{
    NSError *error = [request error];
    NSLog(@"error: %@", [error localizedFailureReason]);
    [self.view makeToast:@"添加联系人失败，请检查网络及手机是否工作正常。"];
}

- (void)del_contact_finished:(ASIHTTPRequest *)request
{
}

- (void)del_contact_failed:(ASIHTTPRequest *)request
{
    NSError *error = [request error];
    NSLog(@"error: %@", [error localizedFailureReason]);
    [self.view makeToast:@"删除联系人失败，请检查网络及手机是否工作正常。"];
}
