...
@interface BTTaskCreateViewController ()

@property NSMutableArray* admins;

@property NSMutableArray* peoples;

@end

@implementation BTTaskCreateViewController
...
- (IBAction)select_admin:(id)sender {
    self.mRequestId = BTTaskSelectAdmin;
...
- (IBAction)select_people:(id)sender {
    self.mRequestId = BTTaskSelectPeople;
    BTFriendFindViewController *findfriendTableViewController = [[BTFriendFindViewController alloc] init];
    findfriendTableViewController.delegate = self;
    [self.navigationController pushViewController:findfriendTableViewController animated:YES];
}
...
- (IBAction)save_meeting:(id)sender {
    ...
    {
        [self.view makeToast:@"任务名称不能为空。"];
        return;
    }
    
    AppDelegate *delegate=(AppDelegate*)[[UIApplication sharedApplication]delegate];
    
    NSString *strURL = [@"" stringByAppendingFormat:@"%@%@%@", @"http://", delegate._server, @"/tasks/"];
...
-(void) create_meeting_failed:(ASIHTTPRequest *)request
{
    ...
    NSLog(@"保存任务异常: %@", [error localizedFailureReason]);
    [self.view makeToast:@"保存任务异常，请检查网络及设备，如仍无法解决，请联系我们。"];
}
