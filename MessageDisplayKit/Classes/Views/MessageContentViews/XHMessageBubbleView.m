...
@property (nonatomic, weak, readwrite) UITextView *displayTextView;

@property (nonatomic, weak, readwrite) UILabel *displayRichTextView;
...
+ (CGSize)neededSizeForText:(NSString *)text {
    ...
    CGFloat cwidth = (dyWidth > textSize.width ? textSize.width : dyWidth) + kBubblePaddingRight * 2 + kXHArrowMarginWidth;
    return CGSizeMake(cwidth > 40 ? cwidth : 40, textSize.height > 40 ? textSize.height : 40);// 蒲剑修改
}
...
- (instancetype)initWithFrame:(CGRect)frame
    ...
    if (self) {
        ...
        if (!_displayTextView) {
            UITextView *displayTextView = [[UITextView alloc] initWithFrame:CGRectZero];
