...
@property (nonatomic, weak, readwrite) UILabel *senderLabel; // 蒲剑增加

@property (nonatomic, weak, readwrite) LKBadgeView *timestampLabel;

@property (nonatomic, weak, readwrite) UILabel *timeLabel; // 蒲剑增加
...
- (instancetype)initWithMessage:(id <XHMessageModel>)message
    ...
    if (self) {
        ...
        {
            timeLabelFrame = CGRectMake(272,10,40,40);
        }
        else
        {
           timeLabelFrame = CGRectMake(8,10,40,40);
        }
        ...
        switch (message.bubbleMessageType) {
            case XHBubbleMessageTypeReceiving:
                avatorButtonFrame = CGRectMake(8,10,40,40);
                break;
            case XHBubbleMessageTypeSending:
                avatorButtonFrame = CGRectMake(272,10,40,40);
                break;
            default:
                break;
        }
        ...
        // 蒲剑 原框架无发送者姓名功能，增加显示发送者姓名
        // CGRect senderLabelFrame = CGRectMake(14, 54, 40, 20);
        
        CGRect senderLabelFrame;
        switch (message.bubbleMessageType) {
            case XHBubbleMessageTypeReceiving:
                senderLabelFrame = CGRectMake(8,  54, 42, 20);
                break;
            case XHBubbleMessageTypeSending:
                senderLabelFrame = CGRectMake(270,  54, 42, 20);
                break;
            default:
                break;
        }
        
        UILabel *senderLabel = [[UILabel alloc] initWithFrame:senderLabelFrame];
        senderLabel.font = [UIFont fontWithName:@"Helvetica" size:14];
        [senderLabel setTextColor:[UIColor grayColor]];
        [senderLabel setText:[message sender]];
        [senderLabel setTextAlignment:NSTextAlignmentCenter];
