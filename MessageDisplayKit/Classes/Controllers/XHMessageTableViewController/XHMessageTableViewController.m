...
- (void)initilzer {
    ...
    [messageTableView.tableHeaderView addSubview:titleLabel];
...
- (void)didSendEmotionMessageWithEmotion:(XHEmotion *)emotion {
    DLog(@"send emotion : %@", emotion);
    if ([self.delegate respondsToSelector:@selector(didSendEmotion:fromSender:onDate:)]) {
        [self.delegate didSendEmotionN:emotion fromSender:self.messageSender onDate:[NSDate date]];
    }
}
